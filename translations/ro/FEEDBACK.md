# Reader feedback — Romanian edition (v2)
...
`echo` (touches a repeated phrase), `question` (answers an open item in `translations/ro/NOTES.md` §H).
...
```
.venv/bin/python tools/check_echoes.py ro --check
```

after the phase-two pass, and re-snapshot if a guarded phrase is deliberately
retired.

`translations/ro/NOTES.md` §H already carries **12 open questions** for this edition.
