# Reader feedback — Hungarian edition (v1)
...
`echo` (touches a repeated phrase), `question` (answers an open item in `translations/hu/NOTES.md` §H).
...
```
.venv/bin/python tools/check_echoes.py hu --check
```

after the phase-two pass, and re-snapshot if a guarded phrase is deliberately
retired.

`translations/hu/NOTES.md` §H already carries **10 open questions** for this edition.
