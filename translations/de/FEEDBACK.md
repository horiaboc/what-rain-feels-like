# Reader feedback — German edition (v3)
...
`echo` (touches a repeated phrase), `question` (answers an open item in `translations/de/NOTES.md` §H).
...
```
.venv/bin/python tools/check_echoes.py de --check
```

after the phase-two pass, and re-snapshot if a guarded phrase is deliberately
retired.

`translations/de/NOTES.md` §H already carries **13 open questions** for this edition.
