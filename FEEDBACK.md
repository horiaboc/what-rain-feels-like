# Reader feedback — English edition
...
`echo` (touches a repeated phrase), `question` (answers an open item in the notes §H).
...
```
.venv/bin/python tools/check_echoes.py en --check
```

after the phase-two pass, and re-snapshot if a guarded phrase is deliberately
retired.

This edition has no `NOTES.md` §H list, because its decisions were not
